...
"""
Photo ingestion script - moves photos from imports to organized directories.

Reads metadata from photos in photos/imports/ and moves them to:
...
def ingest_photo(photo_path, photos_root, dry_run=False):
    ...
    Args:
        photo_path: Path to the photo in imports folder
...
def main():
    ...
    if '--output-paths' in sys.argv:
        i = sys.argv.index('--output-paths')
        if i + 1 < len(sys.argv):
            output_paths_file = Path(sys.argv[i + 1])

    # Get project root (photography directory)
    script_dir = Path(__file__).parent
    project_root = script_dir.parent.parent.parent
    photos_root = project_root / "photos"
    imports_dir = photos_root / "imports"
    ...
    if not imports_dir.exists():
        ...
        print(f"Created: {imports_dir}")
        print("\nPlace photos in this directory and run the script again.")
        return 0

    # Find all image files in imports
    image_extensions = {'.jpg', '.jpeg', '.png', '.gif', '.JPG', '.JPEG', '.PNG', '.GIF'}
    photos = [f for f in imports_dir.iterdir() if f.is_file() and f.suffix in image_extensions]

    if not photos:
        print(f"No photos found in {imports_dir.relative_to(project_root)}")
        return 0
    ...
    print(f"{mode}")
    print(f"{'=' * 60}")
    print(f"Found {len(photos)} photo(s) in imports/\n")

    # Process each photo
    success_count = 0
    error_count = 0
    replaced_count = 0
    ingested_dest_paths: list[Path] = []
    ...
    for photo_path in sorted(photos):
        ...
        if success:
            ...
            if dest_path is not None and not dry_run:
                ingested_dest_paths.append(dest_path)
        else:
            error_count += 1

    if output_paths_file is not None and not dry_run:
        output_paths_file.parent.mkdir(parents=True, exist_ok=True)
        with open(output_paths_file, 'w') as f:
            for p in ingested_dest_paths:
                f.write(f"{p}\n")
